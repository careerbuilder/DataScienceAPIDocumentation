...
* **job_title** (Optional) -- a job title supplied by the user, which will be used to override the job_title field in the response. The job_title_clean filed in the response will be based on this supplied value as well instead of using the parsed from the document. This is useful when user already has the job title along with the document and believes the job title is more accurate than the parsed from the document.

* **location** (Optional) -- a location supplied by the user, which will be used to override job_location field in the response. This is useful when user already has the location along with the document and believes the location is more accurate than the parsed from the document.
