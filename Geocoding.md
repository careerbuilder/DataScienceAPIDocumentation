...
The service also accepts Google Place Id's as a means to retrieve goeographic information. Requesting a location using a Google Place Id requires that a request only contains the Place Id and/or a culture param. Additional location parameters will result 400 response in this kind of request.
...
| place_id | Optinal. Id provided by google that uniquely identify places based on the Google Places Database and on Google Maps
